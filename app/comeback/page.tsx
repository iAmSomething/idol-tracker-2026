    ...
    async function fetchComebackData() {
      try {
        ...
        const tracksData = tracksSnap.docs.map(
          (d) => ({ ...d.data(), id: d.id }) as Track,
        );

        // Deduplicate tracks by name (in case backend deduplication is delayed by quota)
        const uniqueTracks = new Map<string, Track>();
        for (const t of tracksData) {
          const nameKey = (t.name || "").trim().toLowerCase();
          if (!uniqueTracks.has(nameKey)) {
            uniqueTracks.set(nameKey, t);
          } else {
            // Keep the one that has a bugs link or order if the existing one doesn't
            const existing = uniqueTracks.get(nameKey)!;
            const existingScore =
              (existing.streamingLinks?.bugs ? 10 : 0) +
              (existing.order !== undefined ? 1 : 0);
            const newScore =
              (t.streamingLinks?.bugs ? 10 : 0) +
              (t.order !== undefined ? 1 : 0);
        ...
        // Sort by order if available, otherwise fallback to name
    ...
    <>
      <main className="app-container">
        ...
        <div className="detail-bento-grid">
          ...
          >
            {comeback.albumCoverUrl || comeback.artistProfileImageUrl ? (
              <img
                src={comeback.albumCoverUrl || comeback.artistProfileImageUrl}
            ...
            <div>
              ...
              >
                ...
                >
                  ...
                  <span
                    className={`tag-badge ${getTagClass(comeback.releaseType)}`}
                    style={{
                      margin: 0,
                      fontSize: "0.75rem",
                      padding: "4px 8px",
          ...
          >
            ...
            {formattedMvUrl && (
              <div className="bento-card" style={{ padding: "20px" }}>
                ...
                >
                  <FaYoutube color="var(--accent-color)" /> 타이틀곡 뮤직비디오
                  / 티저
                </h3>
                <div
                  style={{
                  ...
                  >
                    ...
                    {comeback.recentNews.map((news, idx) => (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="track-item hover:border-accent"
                        style={{
                          display: "flex",
                          flexDirection: "column",
                          gap: "4px",
                      ...
                      >
                        ...
                        >
                          <span
                            ...
                            style={{ width: "20px", fontWeight: 600 }}
                          >
                            {idx + 1}
                          </span>
                          <Link
                            ...
                            style={{
                              fontWeight: track.isTitle ? 600 : 500,
                              color: "var(--text-primary)",
                              whiteSpace: "nowrap",
                              overflow: "hidden",
